...
2. Apply the Test-List checkpoint from `.pi/skills/exact-coding-tcr/human-in-the-loop.md`.
3. After approval when required, stage the test list, run the full suite, and commit it only if the suite passes:
...
4. For exactly one behavior at a time, follow `.pi/skills/tcrdd/SKILL.md`:
...
`.pi/skills/exact-coding-tcr/human-in-the-loop.md` is the single source of truth for checkpoints. Its default `full-hitl` level stops after Test List, Red, and Refactor. Green has no default stop.
